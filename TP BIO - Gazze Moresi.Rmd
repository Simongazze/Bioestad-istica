...
-   Se contaran las celdas que esten vacias en cada variable.
...
datatable(reglas %>% select(1:3) %>% slice(1:18))
```
...
print(paste("Participantes limpios:", ind_limp))
```
...
```{r}
print(paste("Participantes con inconsistencias:", ind_incos))
```

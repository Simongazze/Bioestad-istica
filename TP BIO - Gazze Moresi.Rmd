...
"r9", "(ethnicgroup) distinto de 1,2,3 o 4", "!ethnicgroup %in% c(1, 2, 3, 4)",
"r10", "(scr) distinto de 1 o 2", "!scr %in% c(1,2)",
"r11", "(usscr) distinto de 1 o 2", "!usscr %in% c(1,2)",
"r12", "(consent) distinto de 1 o 2", "!consent %in% c(1,2)",
"r13", "(countrycode) distinto de 4, 11, 14, 23, 31, 48, 54, 65, 72 o 97", "!countrycode %in% c(4, 11, 14, 23, 31, 48, 54, 65, 72, 97)",
"r14", "(interview) no sigue formato dd/mm/yyyy", "!tryCatch(!is.na(as.Date(`interview`, format='%d/%m/%Y')), error = function(e) TRUE)",
"r15", "Edad fuera de rango según Patient ID y Date of interview", "{
    fecha_nac <- as.Date(sub('-.*', '', `Patient ID`), format='%d/%m/%Y')
    fecha_entrevista <- as.Date(`Date of interview`, format='%d/%m/%Y')
...
"r17", "subjectnumber está vacío cuando scr, usscr y consent son 2", "(scr == 2 & usscr == 2 & consent == 2) & (is.na(subjectnumber) | subjectnumber == '')",
"r18", "subjectnumber no respeta formato: 3 dígitos de countrycode + 6 números", "!grepl('^\\d{9}$', subjectnumber) | substr(subjectnumber, 1, 3) != sprintf('%03d', countrycode)"
)
...
```{r}
validacion = validar(adm, "id", validador(reglas, "id", "cond"))
validacion
```
